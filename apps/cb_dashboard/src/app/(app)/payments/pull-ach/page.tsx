...
const ENTRY_CLASSES = [
  ...
  { value: "TEL", label: "TEL — Telephone Initiated" },
];

export default function PullAchPage() {
  ...
  const submit = async () => {
    if (!partyId || !destId || !amount || submitting) return;
    ...
    setSubmitting(true);
    try {
      const ikey = `ach-pull-${Date.now()}-${Math.random().toString(36).slice(2)}`;
      // Pull ACH: debit external account (source) into our internal account (dest)
      // Using destId as both source and dest for the internal leg until external account support is added
  ...
  const destOptions = partyAccounts.map((a) => ({
    value: a.account_id,
    ...
    <Stack gap="lg">
      ...
      <Card withBorder shadow="sm" radius="md" padding="lg">
        ...
        <Text size="sm" c="dimmed" mb="lg">
          Initiate an ACH debit to pull funds from an external account into an
          internal account.
        </Text>
        ...
        <Stack>
          ...
          <Group grow>
            ...
            <TextInput
              ...
              onChange={(e) => setExternalAccount(e.currentTarget.value)}
              required
            />
          </Group>
          ...
          <Group grow>
            ...
            <Select
              ...
              placeholder="Internal destination account"
              data={destOptions}
              value={destId}
              onChange={setDestId}
              disabled={!partyId}
          ...
          <Group mt="sm">
            <Button
              onClick={submit}
              loading={submitting}
              disabled={!partyId || !destId || !amount}
            >
              Initiate Pull ACH
            </Button>
            <Button

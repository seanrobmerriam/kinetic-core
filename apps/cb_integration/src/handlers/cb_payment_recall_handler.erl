%% @doc Payment Recall Handler
%%
%% Handler for `POST /api/v1/payment-orders/:payment_id/recall`.
%%
%% Recalls (reverses) a completed payment order.
-module(cb_payment_recall_handler).
...
handle(<<"POST">>, PaymentId, Req, State) ->
    case cb_payment_orders:recall_payment(PaymentId) of
        {ok, Order} ->
